                            ...
                            child: Material(
                              ...
                              child: InkWell(
                                ...
                                child: Container(
                                  ...
                                  child: Row(children: [
                                    Icon(icon, size: 20, color: sel ? const Color(0xFF00BCD4) : const Color(0xFF8B99A6)),
    ...
    return Column(
      ...
      children: [
        ...
        if (_loading)
          ...
          const _AdminEmpty(message: 'No bookings yet.')
        else
          for (final b in _bookings) _BookingRow(booking: b, onApprove: _approve, onReject: _reject),
      ],
    );
  }
}
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            ...
            const SizedBox(width: 10),
            Text(ref, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Color(0xFF006994))),
            const Spacer(),
            Text(amount, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: Color(0xFF50C878))),
          ]),
          const SizedBox(height: 10),
          _InfoRow(icon: Icons.email_outlined, text: email.isNotEmpty ? email : 'No email'),
          _InfoRow(icon: Icons.calendar_today, text: '$startDate → $endDate'),
          _InfoRow(icon: Icons.people, text: '$guests guest(s)'),
          if (status == 'pending') ...[
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(children: [
        ...
        Expanded(child: Text(text, style: const TextStyle(fontSize: 13, color: Color(0xFF5D6D7A)))),
      ]),
    );
  }
}
    ...
    return Column(
      ...
      children: [
        ...
        if (_loading)
          ...
          const _AdminEmpty(message: 'No destinations in the database yet. Add one below.')
        else
            ...
            children: [
              if (_success != null) ...[
                Container(
                  ...
                  child: Text(_success!, style: const TextStyle(color: Color(0xFF50C878), fontWeight: FontWeight.w600)),
                ),
              ...
              if (_error != null) ...[
                Container(
                  ...
                  child: Text(_error!, style: const TextStyle(color: Color(0xFFFF6B4A))),
                ),
                const SizedBox(height: 16),
              ],
              _FormField(label: 'Spot Name *', controller: _nameCtrl, hint: 'e.g. Kawasan Falls'),
              const SizedBox(height: 16),
              _FormField(label: 'Description', controller: _descCtrl, hint: 'Brief description', maxLines: 3),
              const SizedBox(height: 16),
              const Text('Region *', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF5D6D7A))),
              ...
              const SizedBox(height: 16),
              Row(children: [
                Expanded(child: _FormField(label: 'Entrance Fee (₱) *', controller: _feeCtrl, hint: '0', keyboardType: TextInputType.number)),
                const SizedBox(width: 16),
                Expanded(child: _FormField(label: 'Latitude', controller: _latCtrl, hint: '10.3157', keyboardType: TextInputType.number)),
                const SizedBox(width: 16),
                Expanded(child: _FormField(label: 'Longitude', controller: _lngCtrl, hint: '123.8854', keyboardType: TextInputType.number)),
              ]),
              const SizedBox(height: 16),
              ...
              SizedBox(
                ...
                child: FilledButton(
                  ...
                  child: _saving
                      ...
                      : const Text('Add Spot', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 15)),
                ),
              ),
            ],
          ),
...
class _FormField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(label, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF5D6D7A))),
        const SizedBox(height: 8),
        TextField(
...
class _AdminEmpty extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Center(child: Text(message, style: const TextStyle(color: Color(0xFF8B99A6)))),
    );
  }
}
