...
class _DriverScreenState extends State<DriverScreen> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Trip marked as completed. Guest notified.'),
          ),
        );
      }
    } catch (e) {
  ...
  List<Map<String, dynamic>> get _filtered {
    ...
    if (_statusFilter != 'all') {
      list = list
          .where((b) => (b['status'] as String? ?? '') == _statusFilter)
          .toList();
    }
    ...
    if (_filterDate != null) {
      list = list.where((b) {
        final start = DateTime.tryParse(
          (b['start_date'] as String? ?? '').split('T').first,
        );
        final end = DateTime.tryParse(
          (b['end_date'] as String? ?? '').split('T').first,
        );
      ...
      children: [
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  ...
                  focusedBorder: OutlineInputBorder(
                    borderRadius: BorderRadius.circular(10),
                    borderSide: const BorderSide(color: _kOcean),
                  ),
                ),
              ),
            ),
        ...
        if (_filtered.isEmpty)
          Container(
            ...
            child: Column(
              children: [
                ...
                Text(
                  _trips.isEmpty
                      ? 'Your admin will assign bookings to you.'
          ...
          ListView.builder(
            ...
            itemBuilder: (_, i) =>
                _TripCard(booking: _filtered[i], onComplete: _complete),
          ),
      ],
    );
  }
...
class _TripCardState extends State<_TripCard> {
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (transId.isNotEmpty &&
                      _transLabels.containsKey(transId)) ...[
                    _infoRow(
                      Icons.directions_car_outlined,
                      'Transport',
                      _transLabels[transId]!,
                    ),
                    const SizedBox(height: 8),
                  ],
  ...
  Widget _iconRow(IconData icon, String text) => Row(
    children: [
      ...
      const SizedBox(width: 6),
      Expanded(
        child: Text(text, style: const TextStyle(fontSize: 13, color: _kDark)),
      ),
    ],
  );
    ...
    try {
      ...
      if (results.isNotEmpty && !_scanned) {
        final raw = results.first
            .getProperty<JSString?>('rawValue'.toJS)
            ?.toDart;
